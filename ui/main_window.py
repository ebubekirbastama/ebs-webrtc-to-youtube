...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._build(); self._load(); self._check_ffmpeg(); self._refresh_secret_state()
    def _build(self):
    ...
    def _message(self, icon, title, text):
        ...
        box.setStyleSheet('QMessageBox { background:#f4f6f8; color:#26313a; } QMessageBox QLabel { color:#26313a; background:transparent; } QMessageBox QPushButton { color:#26313a; background:#ffffff; border:1px solid #aeb6bf; border-radius:8px; min-width:80px; min-height:34px; font-weight:700; } QMessageBox QPushButton:hover { background:#eef3f7; }')
        return box.exec()
    ...
    def start_bridge(self):
        ...
        if not offer or not rtmp or not ff: self.warn('Eksik bilgi','Offer, RTMP ve FFmpeg alanları zorunlu.'); return
        ...
        except Exception as e: self.error('Başlatma hatası',str(e))
    ...
    def _async(self,fn,ok):
        th=QThread(self); w=Worker(fn); w.moveToThread(th); th.started.connect(w.run); w.done.connect(ok); w.done.connect(th.quit); w.fail.connect(lambda e:(self.log('[YouTube Hata] '+e),self.error('YouTube',e))); w.fail.connect(th.quit); th.finished.connect(w.deleteLater); th.finished.connect(th.deleteLater); th.start(); self._thread=th

    def _refresh_secret_state(self):
        if SECRET.is_file():
            token = ' • OAuth kayıtlı' if TOKEN.is_file() else ''
            self.secret_label.setText('client_secret.json: HAZIR' + token)
        else:
            self.secret_label.setText('client_secret.json: BULUNAMADI')
    ...
    def pick_client_secret(self):
        ...
        try:
            ...
            self.log('[YouTube] client_secret.json güvenli şekilde proje köküne kopyalandı.')
    ...
    def youtube_disconnect(self):
        disconnect()
        self.yt=None
        self.ytstatus.set_state('idle','● Bağlı değil')
        self._refresh_secret_state()
    ...
    def youtube_fetch(self):
        ...
        def ok(r):
            ...
            if not items: self.info('YouTube','Mevcut stream bulunamadı. Yeni yayın oluşturabilirsiniz.'); return
